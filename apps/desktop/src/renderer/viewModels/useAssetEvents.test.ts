...
import { subscribeToAssetEvents, useAssetEvents } from './useAssetEvents';
...
describe('useAssetEvents', () => {
  ...
  it('subscribes through the gateway helper', () => {
    ...
    expect(subscribeToAssetEvents(listener)).toBe(unsubscribe);
    expect(onEventMock).toHaveBeenCalledWith(listener);
  });

  it('cleans up the asset subscription on unmount', () => {
    const unsubscribe = vi.fn();
    const listener = vi.fn();

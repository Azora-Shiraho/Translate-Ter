  ...
  return (
    <div className="exportGroup">
      ...
      <div className="exportActions">
        <button disabled={props.disabled.translated} onClick={() => props.onExport('translated')} type="button">
          {props.exportingVariant === 'translated' ? <Download size={14} className="spin" /> : <ArrowUpRight size={14} />}
          {props.labels.translated}
        </button>
        <button disabled={props.disabled.source} onClick={() => props.onExport('source')} type="button">
          {props.exportingVariant === 'source' ? <Download size={14} className="spin" /> : <ArrowUpRight size={14} />}
          {props.labels.source}
        </button>
        <button disabled={props.disabled.bilingual} onClick={() => props.onExport('bilingual')} type="button">

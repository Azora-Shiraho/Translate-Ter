...
方案已在 PR #29 的绑定分支落地：共享模型增加兼容的结构化用户消息描述，Renderer 统一解析用户文案与技术详情，provider health、资产、任务、批量队列和字幕 warning 已开始产出稳定翻译键。Native 命令和协议版本保持不变，旧 `message` 字段继续作为兼容与诊断回退。
